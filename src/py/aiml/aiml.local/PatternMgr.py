...
class PatternMgr(object):
	...
	def __init__(self):
		...
		## self._whitespaceRE = re.compile("\s+", re.LOCALE | re.UNICODE) ## python3
	...
	def setBotName(self, name):
		...
		## self._botName = str(string.join(name.split())) ## python3
	...
	def add(self, xxx_todo_changeme, template):
		...
		node = self._root
		#PYTHON2: for word in string.split(pattern):
		...
		if len(that) > 0:
			...
			#PYTHON2: for word in string.split(that):
		...
		if len(topic) > 0:
			...
			#PYTHON2: for word in string.split(topic):
	...
	def match(self, pattern, that, topic):
		...
		## python3: topicInput = string.upper(topic)
	...
	def star(self, starType, pattern, that, topic, index):
		...
		# Mutilate the input.  Remove all punctuation and convert the
		# text to all caps.
		#PYTHON2: input = string.upper(pattern)
		input = pattern.upper()
		input = re.sub(self._puncStripRE, " ", input)
		input = re.sub(self._whitespaceRE, " ", input)
		if that.strip() == u"": that = u"ULTRABOGUSDUMMYTHAT" # 'that' must never be empty
		#PYTHON2: thatInput = string.upper(that)
		thatInput = that.upper()
		thatInput = re.sub(self._puncStripRE, " ", thatInput)
		thatInput = re.sub(self._whitespaceRE, " ", thatInput)
		if topic.strip() == u"": topic = u"ULTRABOGUSDUMMYTOPIC" # 'topic' must never be empty
		#PYTHON2: topicInput = string.upper(topic)
		topicInput = topic.upper()
		topicInput = re.sub(self._puncStripRE, " ", topicInput)
		...
		if foundTheRightStar:
			...
			#PYTHON2: if starType == 'star': return string.join(pattern.split()[start:end+1])
			#PYTHON2: elif starType == 'thatstar': return string.join(that.split()[start:end+1])
			#PYTHON2: elif starType == 'topicstar': return string.join(topic.split()[start:end+1])
