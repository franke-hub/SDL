...
_thread = None                      ## The _TestServerThread, when running
...
def _logger(*args, **kwargs):
    ...
    log = open("log/test-server.log", "a")
    log.write(line)
    log.close()

def _server(*args, **kwargs):
    debugf("TEST server:", *args, **kwargs)

def _transmit(sess, mess):
    try:
        tab = control['post-server']
        uow = UOW(WDO=WDW())
        uow.sess = sess
        uow.work = mess
        ...
        resp = "Internal logic error: " + str(x)

    return resp

##############################################################################
## The Test request handler: Created for each page served
##############################################################################
class _TestRequestHandler(_SimpleHTTPRequestHandler):
    global _thread
...
##      self.send_header("Last-Modified", self.date_time_string())
    ...
    def do_POST(self):
        ...
        messout = _transmit(session, messinp)
...
##############################################################################
## _TestRemoveThread: Terminates the _TestServerThread
##############################################################################
class _TestRemoveThread(threading.Thread):
    def __init__(self, _thread):
        super(_TestRemoveThread, self).__init__()
        self.name = "TestRemove"
        self.daemon = True
...
##############################################################################
## _TestServerThread: Operates the _TCPServer
##############################################################################
class _TestServerThread(threading.Thread):
    def __init__(self):
        super(_TestServerThread, self).__init__()
    ...
    def run(self):
        ...
        try:
            self._httpd = _TCPServer(("",_PORT), _TestRequestHandler)
            _server("Running, port:", _PORT)
            _logger("%.3f"%time.time(), time.asctime(),
                    "TEST server: Running, port:", _PORT)
    ...
    def stop(self):
        if self._del_thread():
            _TestRemoveThread(_thread)

##############################################################################
## __Command class: (test-server command)
##############################################################################
...
class __Command:
        ...
        if argc > 1:
            if argv[1].upper() == 'PORT':
                ...
                if running:
                    ...
                    _TestServerThread() ## (Sets _thread before return)
                    time.sleep(0.125)
    ...
    @staticmethod
    def work(arg):
        command['test-server'].run(['test-server', arg])

command['test-server'] = __Command

##############################################################################
## Startup: start the Test server
##############################################################################
if False:                           ## MARKER
    _TestServerThread()             ## Start the test-server thread
