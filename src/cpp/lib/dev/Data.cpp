...
#include "pub/http/utility.h"       // For pub::http::utility::visify_char
...
,  BUFFER_SIZE= 16'384              // The maximum buffer length
,  VERIFY= true                     // Use internal consistency checking?
}; // enum
...
{  if( HCDM )
   ...
   Data::Data(                    // Copy onstructor
     const Data&       from)      // (The copy)
:  list()
{  if( HCDM )
...
{  if( HCDM )
   ...
   size_t length= size;             // Remaining length
   string output;                   // The output string
   char buffer[2048];               // Work buffer
//   char buffer[8];                  // Work buffer (for testing) SHCDM

   while( length >= sizeof(buffer) ) { // If segments remain
     size_t L= store(buffer, sizeof(buffer), offset);
     if( VERIFY && L != sizeof(buffer) )
       throwf("%4d %s SHOULD NOT OCCUR %zd\n", __LINE__, __FILE__, L);

     string segment(buffer, sizeof(buffer));
     output += segment;
     offset += sizeof(buffer);
     length -= sizeof(buffer);
   }

   // The last store adds the trailing blank
   size_t L= store(buffer, length + 1, offset);
   if( VERIFY && L != length )
     throwf("%4d %s SHOULD NOT OCCUR %zd != %zd\n", __LINE__, __FILE__
           , L, length);
   string segment(buffer, length);
   if( offset )
     output += segment;
   else
...
size_t                              // The number of bytes stored
   Data::store(                     // Store (copy) the Data
     ...
     size_t            offset) const // At this offset
{  if( HCDM )
     debugh("Data(%p)::store(%p,%zd,%zd)\n", this, addr_, size, offset);
...
#if 0  // Note: method names are unqualified
virtual char*                       // The next character
   peek( void ) const               // Get next character
{  return nullptr; }                // Let's see if it's needed

virtual char*                       // The next character
   seek(                            // Set offset (store)
     size_t            offset)      // To this value (from origin)
{  (void)offset; return nullptr; }  // (Object not operational)

virtual int                         // Return code (0 OK)
   store(                           // Store (copy) the buffer
     char*             addr,        // From this address
     size_t            size)        // For this length
{  (void)addr; (void)size; return -1; } // (Object not operational)

virtual size_t                      // The current offset (from origin)
   tell( void ) const               // Get current offset
{  return 0; }                      // (Object not operational)
#endif
...
//
// Implementation notes-
//       POSSIBLE TODO: Remove debugging; move methods inline.
//
//----------------------------------------------------------------------------
   Buffer::~Buffer( void )          // Destructor
{  if( HCDM ) debugh("~Buffer(%p) @%p\n", this, addr); delete addr; }

   Buffer::Buffer(                  // Constructor
     size_t            size)        // Buffer size
:  addr(new char[size == 0 ? size_t(BUFFER_SIZE) : size])
,  size(size == 0 ? size_t(BUFFER_SIZE) : size)
{  if( HCDM ) debugh("Buffer(%p)(%zd) @%p\n", this, this->size, addr); }
...
//
// Implementation notes-
//       On buff length exceeded, std::length_error thrown
//
//----------------------------------------------------------------------------
void
...
{  size_t append_size= strlen(S);
   if( (length + append_size) > size )
     throw std::length_error("append c_string");

   memcpy(addr+length, S, append_size);
   length += append_size;
}
...
{  size_t append_size= S.size();
   if( (length + append_size) > size )
     throw std::length_error("append std::string");

   memcpy(addr+length, S.c_str(), append_size);
   length += append_size;
}
...
{  if( HCDM ) debugh("Buffer(%p)::fetch(%p,%zd)\n", this, &data, offset);
   ...
   offset= 0;
   length= data.store(addr, size, offset);
}
...
{  if( HCDM && VERBOSE > 1) {
     ...
     string S= utility::visify_char(C);
