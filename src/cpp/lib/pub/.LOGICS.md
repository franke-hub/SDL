...
It is a self-contained header file.
While there is a Latch.cpp file, it's only conditionally used for debugging.
...
#### NullLatch
The NullLatch was a special-purpose single thread optimizing latch.
Since no use case exists, it's been removed.

#### TestLatch
The TestLatch was a special purpose debugging Latch that explicitly disallows
recursion, thus detecting Latch self-deadlocks.

Since Latch now provides this function, it's been removed.
