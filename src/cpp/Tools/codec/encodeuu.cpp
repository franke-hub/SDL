...
//
// Purpose-
//       Convert a file into its UU encoding.
//
// Last change date-
//       2007/01/01
//
...
{
   fprintf(stderr, "encodeuu filename ... >output-filename\n");
   fprintf(stderr, "\n");
     ...
     const char*       fileName)    // -> FileName to encode
{
   ...
   UuCodeCodec         codec;       // Codec object
     ...
     char*             argv[])      // Argument array
{
   ...
   for(i=1; i<argc; i++)
   {
     ...
     rc= encodeuu(argv[i]);         // Encode from file
     if( rc != 0 )                  // If failure
