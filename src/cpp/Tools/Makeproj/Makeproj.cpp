...
//
// Function-
//       Examine a source file and any files it refers to in order to
     ...
     Entity*         inpEntity)     // -> Entity
{
   ...
   while( fgets(inpBuff, sizeof(inpBuff)-1, inpFile) != NULL ) // Read a line
   {
     ...
     while( *ptrC == ' ' )          // Skip leading blanks
       ptrC++;

     if (memcmp(ptrC, "#include", 8) == 0) // Include statement
     {
       ptrC += 8;                  // Skip over the #include
       while( *ptrC == ' ' || *ptrC == '\t' ) // Skip over white space
         ptrC++;
