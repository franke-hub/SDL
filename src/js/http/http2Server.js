...
//
// Usage note-
//       From ~/obj/js/http: make http2Server
//
//----------------------------------------------------------------------------
'use strict';
...
//----------------------------------------------------------------------------
const LOGFILE= 'log/http2Server.log'; // The log file name
//    LOGFILE= '';                  // Comment out to log
const MAXBODY= 0x00040000;          // Largest allowed html body
const PROGRAM= 'http2Server.js';    // The program name
...
//----------------------------------------------------------------------------
// Load the security certificates, also allowing ALPN protocol
//----------------------------------------------------------------------------
var opts= null;
try {
   opts=
   ...
   console.log('Error reading certificates:', err.message);
   process.exit(1);
}
...
if( LOGFILE ) {
   ...
   .then( out => {
     ...
     logger('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
     const time= dt.create().format('d/n/Y H:M:S');
     logger(`>> [${time}] HTTP2 Server started`);
   })
   .then(() => {
