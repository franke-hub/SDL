...
//
// Title-
//       ~/doc/cpp/pub_list-SHSL.md
//
// Purpose-
//       List.h reference manual: SHSL_list<T>
//
// Last change date-
//       2025/10/05
//
...
See also: [`SHSL_list<T>::iterator`](./pub_list-SHSL_iter.md)

#### Definition: "link set"
A link set is a set of links bounded by a head and a tail link.
While a SHSL_list<T> contains an entire link set, portions of the list
...
#### Types:
- `SHSL_list<T>::struct Link`: The Link type

`SHSL_list<T>` Links are implemented using
`class T : public pub::SHSL_list<T>::Link {...};`
...
---
#### <a id="size">size_t size(void) const</a>
Iterates through the list, returning the element count.

---
### <a id="example">Example:</a> Compare with [SHSL_sort example](./pub_list-SORT.md#example)
```
#include <assert.h>                 // For assert
#include <stdio.h>                  // For printf
#include <stdlib.h>                 // For size_t

#include <pub/List.h>               // For PUB::SHSL_list, ...
#define PUB _LIBPUB_NAMESPACE

struct Item : public pub::SHSL_list<Item>::Link {
size_t                 value;

   Item(size_t v) : value(v) {}

   // This method is used by the sort method to sort the List by value
   virtual bool operator<(const Item& that) const override
   { return value < that.value; }
}; // class Item

int main() {
   pub::SHSL_list<Item> list;
