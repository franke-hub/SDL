...
//
// Title-
//       ~/doc/cpp/pub_list-SORT.md
//
// Purpose-
//       List.h reference manual: DHDL_sort<T>
//
// Last change date-
//       2025/10/05
//
...
## `pub::DHDL_sort<T>` a.k.a. `pub::Sort<T>`

See also: [`DHDL_sort<T>::iterator`](./pub_list-SORT_iter.md)
...
#### Types:
- `DHDL_sort<T>::struct Link`: The Link type

`DHDL_sort<T>` Links are implemented using
`class T : public pub::DHDL_sort<T>::Link {...};`
...
#include <pub/List.h>               // For PUB::DHDL_sort, ...
#define PUB _LIBPUB_NAMESPACE

struct Item : public pub::DHDL_sort<Item>::Link {
size_t                 value;
...
int main() {
   ...
   list.fifo(more&);
   list.fifo(two&);
   list.sort();                     // Convert to numeric order by value
